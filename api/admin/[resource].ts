...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (resource === 'conversations') {
    ...
    if (clerkId) {
      ...
      userIdFilter = (matchedUsers ?? []).map((u: { id: string }) => u.id);
    }
    ...
    const uniqueUserIds = [...new Set((convs ?? []).map((c: { user_id: string }) => c.user_id))];
    ...
    const conversations = (convs ?? []).map((c: { id: string; user_id: string }) => ({
      ...c,
